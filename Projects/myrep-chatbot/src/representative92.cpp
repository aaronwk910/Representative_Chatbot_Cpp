...
string representative::processQuestion(string x){//takes in queries and feeds the correct request to answerQuestion
	...
	for(int i=0;i<t;i++){
		if(regex_match(words[i], regex("(.*)what(.*)"))||regex_match(words[i], regex("(.*)district(.*)"))||regex_match(words[i], regex("(.*)do(.*)"))||regex_match(words[i], regex("(.*)you(.*)"))||regex_match(words[i], regex("(.*)support(.*)"))||regex_match(words[i], regex("(.*)for(.*)"))||regex_match(words[i], regex("(.*)q(.*)a(.*)")))
			m++;
	}
	currentRatio=m/t;
	if(currentRatio>0.7)
